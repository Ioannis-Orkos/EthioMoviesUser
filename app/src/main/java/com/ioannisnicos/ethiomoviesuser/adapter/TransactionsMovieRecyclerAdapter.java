...
public class TransactionsMovieRecyclerAdapter extends RecyclerView.Adapter<TransactionsMovieRecyclerAdapter.TransactionViewHolder> {

    private Context mContext;
    private List<TransactionMovieResponse> mTransationList;

    private OnTransactionDelButtonClickListener              mTransactionDelButtonListener;

    public interface OnTransactionDelButtonClickListener {
        void onTransactionDelAcceptButtonClick(int position);
         }

    public void setOnTransactionDelButtonClickListener(OnTransactionDelButtonClickListener listener) {
        mTransactionDelButtonListener = listener;
    }


    public TransactionsMovieRecyclerAdapter(Context context, List<TransactionMovieResponse> transations) {
        mContext = context;
    ...
    public TransactionViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new TransactionsMovieRecyclerAdapter.TransactionViewHolder(LayoutInflater.from(mContext).inflate(R.layout.card_transaction_movie, parent, false));
    }


    @Override
    public void onBindViewHolder(@NonNull TransactionsMovieRecyclerAdapter.TransactionViewHolder holder, int position) {

        holder.mProgressBar.setVisibility(View.GONE);
        Glide.with(mContext.getApplicationContext()).load(mTransationList.get(position).getMovies().getPoster())
                .centerCrop()
        ...
        holder.movieTitleText.setText(mTransationList.get(position).getMovies().getTitle());
