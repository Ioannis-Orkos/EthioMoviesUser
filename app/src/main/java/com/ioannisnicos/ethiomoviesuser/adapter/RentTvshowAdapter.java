...
public class RentTvshowAdapter extends RecyclerView.Adapter<RentTvshowAdapter.RentMovieHolder> {

    private WeakReference<Context> mContext;
    private List<StoresAdditionalStatus> mStoresTvshowStatus;
    private List<Stores> mStores;

    private OnItemClickListener mListener;

    HashMap<Integer, RentTvshowAdapter.RentMovieHolder> mHolderMap=new HashMap<Integer,RentTvshowAdapter.RentMovieHolder>();
    ...
    public RentTvshowAdapter(Context context, List<Stores> stores, List<StoresAdditionalStatus> storesMovieStatus) {
        mContext = new WeakReference<>(context);
        mStores = stores;
        mStoresTvshowStatus = storesMovieStatus;
    }

    @Override
    public RentTvshowAdapter.RentMovieHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new RentTvshowAdapter.RentMovieHolder(LayoutInflater.from(mContext.get()).inflate(R.layout.card_rent_movie, parent, false));
    }

    @Override
    public void onBindViewHolder(RentTvshowAdapter.RentMovieHolder holder, int position) {
        ...
        holder.textReqMesg.setText(mStoresTvshowStatus.get(position).getReq_user_msg());

        String aval = mStoresTvshowStatus.get(position).getTvshow_available();
        ...
        if( mStoresTvshowStatus.get(position).getTvshow_status()==null && mStoresTvshowStatus.get(position).getTvshow_available()==null )    {
            holder.rentButton.setText("Sent request");      }
        else if( mStoresTvshowStatus.get(position).getTvshow_status()==null && mStoresTvshowStatus.get(position).getTvshow_available() !=null )    {
            holder.rentButton.setText("Rent");        }
        else if( mStoresTvshowStatus.get(position).getTvshow_status().equals("ready") && mStoresTvshowStatus.get(position).getTvshow_available()!=null )    {
            holder.rentButton.setText("Ready for collection");
            holder.rentButton.setEnabled(false);
            holder.rentButton.setBackgroundColor(Color.GREEN); }
        else if( mStoresTvshowStatus.get(position).getTvshow_status().equals("pending") && mStoresTvshowStatus.get(position).getTvshow_available()==null )    {
        ...
        //holder.rentButton.setEnabled(false);
        else if ( mStoresTvshowStatus.get(position).getTvshow_status().equals("rejected") && mStoresTvshowStatus.get(position).getTvshow_available()!=null )    {
            holder.rentButton.setText("Request Rejected");
            holder.rentButton.setEnabled(false);       }

    }

    public RentTvshowAdapter.RentMovieHolder  getHolder(int position) {
